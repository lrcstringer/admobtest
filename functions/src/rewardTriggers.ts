 ...
 */
export const onRewardItemWritten = onDocumentWritten({ document: "rewardItems/{itemId}", retry: true, labels: { area: "rewards" } }, async (event) => {
  ...
  if (!campaignId) {
    logger.warn("rewardItem has no campaignId", {
  ...
  try {
    ...
    logger.error("Failed to update campaign counters via trigger", {
      campaignId,
      itemId: event.params.itemId,
      oldStatus,
      newStatus,
    ...
    try {
      ...
      if (campaignDoc.exists) {
        ...
        if (
          ...
          logger.info("Campaign auto-exhausted via trigger", {
            campaignId,
          });
        }
      }
      ...
      logger.warn("Failed to check campaign exhaustion", {
        campaignId,
        error: err,
      });
    }
