...
export default function WalletSendAmount() {
  const [, params] = useRoute("/wallets/:id/send/:contactId");
    ...
    <MobileFrame className="bg-background flex flex-col h-screen">
      ...
      <Dialog open={showSuccess} onOpenChange={(open) => !open && handleDone()}>
        <DialogContent className="bg-card border-white/10 text-white w-[90%] rounded-3xl p-8 flex flex-col items-center justify-center">
           ...
           <h2 className="text-2xl font-bold text-white mb-2 text-center">Money Sent!</h2>
           <p className="text-muted-foreground text-center mb-6">
             You successfully sent <span className="text-white font-bold">R{amount}</span> to {contact?.name}.
           </p>
