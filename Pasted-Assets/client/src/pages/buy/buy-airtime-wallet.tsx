...
import { ArrowLeft, Wallet, ChevronRight, ShoppingBag, Loader2 } from "lucide-react";
...
export default function BuyAirtimeWallet() {
      ...
      <div className="flex flex-col h-full bg-background">
        ...
        <div className="p-6 space-y-4">
          {wallets.map((wallet, index) => (
            <div key={wallet.id}>
              {wallet.qualifies ? (
                <Link href={`/buy/airtime/provider/${wallet.id}`}>
                  <motion.div
                ...
                >
                  <div className="p-5 pl-7 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-bold text-white text-base">{wallet.name}</h3>
                        <p className="text-xs text-muted-foreground mt-1">Not eligible for Airtime</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
